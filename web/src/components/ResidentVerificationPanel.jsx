...
const CATEGORY_OPTIONS = [
  { value: 'PROFILE_PHOTO', label: 'Profile Photo', expectsPhotoOnly: true },
  { value: 'VALID_ID', label: 'Valid ID', expectsPhotoOnly: false },
  { value: 'PROOF_OF_RESIDENCY', label: 'Proof of Residency', expectsPhotoOnly: false },
  { value: 'OTHER_SUPPORTING_DOCUMENT', label: 'Other Supporting Document', expectsPhotoOnly: false },
];
...
export default function ResidentVerificationPanel({ user }) {
  const [statusData, setStatusData] = useState(null);
  const [category, setCategory] = useState('VALID_ID');
  const [selectedFile, setSelectedFile] = useState(null);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);

  const allowUpload = useMemo(() => {
    return ['PENDING_VERIFICATION', 'REJECTED', 'APPROVED'].includes(user?.status);
  }, [user?.status]);

  const loadStatus = async () => {
    try {
      const response = await apiService.getResidentVerificationStatus();
  ...
  const validateFile = (file, selectedCategory) => {
    if (!file) {
    ...
    const option = CATEGORY_OPTIONS.find((x) => x.value === selectedCategory);
    if (option?.expectsPhotoOnly && !ACCEPTED_PHOTO_TYPES.includes(file.type)) {
      return 'Profile photo must be JPG, JPEG, or PNG.';
    }

    if (!option?.expectsPhotoOnly && !ACCEPTED_DOC_TYPES.includes(file.type)) {
      return 'Documents must be JPG, JPEG, PNG, or PDF.';
    }
  ...
  const handleUpload = async () => {
    setError('');
    const validationError = validateFile(selectedFile, category);
    if (validationError) {
      setError(validationError);
      return;
    }
    ...
    setLoading(true);
    try {
      await apiService.uploadResidentFile(category, selectedFile);
      setSelectedFile(null);
      await loadStatus();
    } catch (e) {
  ...
  return (
    <div className="card" style={{ marginTop: '20px' }}>
      <div className="card-header">
        <div className="card-title">Resident Verification</div>
      </div>
      ...
      <div style={{ padding: '14px 0', display: 'grid', gap: '12px' }}>
        ...
        <div>
          <strong>Uploaded Files:</strong> {statusData?.fileCount ?? 0}
        </div>
        ...
        {allowUpload && (
          <>
            ...
            <button className="btn-primary" onClick={handleUpload} disabled={loading || !selectedFile}>
              {loading ? 'Uploading...' : 'Upload Supporting File'}
            </button>
          </>
        )}
